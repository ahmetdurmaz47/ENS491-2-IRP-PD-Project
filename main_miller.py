...
# ============================================================
# 0) ROUTE PRINTING HELPERS (unchanged)
# ============================================================
...
def print_solution_routes(model, depot: int = 0, eps: float = 0.5, show_q: bool = True) -> None:
    """Model içine kaydedilmiş referanslardan period period rota yazdırır."""
    if model.SolCount == 0:
...
def load_irppd_from_excel(
    ...
    coords = {depot: (float(meta.get("DepotX", 0.0)), float(meta.get("DepotY", 0.0)))}
    r = {}

    for idx, row in expanded.iterrows():
        i = idx + 1
        coords[i] = (float(row["X"]), float(row["Y"]))
        r[i] = float(row["ri"])
        L[i] = float(row["LB"])
        U[i] = float(row["UB"])
        I0[i] = float(row["I0"])
...
# ============================================================
# 2) IRP-PD MODEL + MTZ
#    FIXED: For each (i,t), allow ONLY ONE of pickup/delivery (or none)
# ============================================================

def build_irp_pd_mtz_from_excel(data):
    N = data["N"]
    T = data["T"]
    Q = data["Q"]
    m = data["m"]
    ...
    # K_i = min(Q, U_i - L_i)
    K = {i: min(Q, U[i] - L[i]) for i in N}

    model = Model("IRP_PD_MTZ_FROM_EXCEL")
    ...
    for t in T:
        model.addConstr(
            quicksum(x[depot, j, t] for j in N if (depot, j) in A) == y0[t],
            name=f"dep_out_{t}"
        )
    ...
    for t in T:
        for i in N_all:
            model.addConstr(
                quicksum(x[i, j, t] for j in N_all if (i, j) in A) -
                quicksum(x[j, i, t] for j in N_all if (j, i) in A) == 0,
                name=f"flow_{i}_{t}"
            )
    ...
    for t in T:
        for i in N:
            model.addConstr(
                quicksum(x[i, j, t] for j in N_all if (i, j) in A) == y[i, t],
                name=f"visit_{i}_{t}"
            )

    # inventory balance at customers (works for any sign d)
    # I[i,t] = I[i,t-1] + q_plus - q_minus - d
    for t in T:
        for i in N:
            model.addConstr(
                I[i, t] - I[i, t - 1] - q_plus[i, t] + q_minus[i, t] + d[(i, t)] == 0,
                name=f"inv_cust_{i}_{t}"
            )
    ...
    # bounds on inventories at customers
    for t in T:
        for i in N:
            model.addConstr(I[i, t] >= L[i], name=f"lb_{i}_{t}")
            model.addConstr(I[i, t] <= U[i], name=f"ub_{i}_{t}")

    # service amount limits + link to visit
    for t in T:
        for i in N:
            model.addConstr(q_plus[i, t] <= K[i] * y[i, t], name=f"serv_plus_{i}_{t}")
            model.addConstr(q_minus[i, t] <= K[i] * y[i, t], name=f"serv_minus_{i}_{t}")

    # ---- FIX: enforce "either pickup OR delivery OR none" based on sign of d(i,t) ----
    # If d>0 -> delivery period => q_minus = 0
    # If d<0 -> pickup period   => q_plus  = 0
    # If d=0 -> no-service      => q_plus = q_minus = 0  (cleanest)
    for t in T:
        for i in N:
            dit = d[(i, t)]
            if dit > 0:
    ...
    for t in T:
        for i in N:
            model.addConstr(
                quicksum(l[j, i, t] for j in N_all if (j, i) in A) +
                q_minus[i, t] -
                q_plus[i, t] -
                quicksum(l[i, j, t] for j in N_all if (i, j) in A) == 0,
                name=f"load_{i}_{t}"
            )

    # total load leaving depot limited by depot inventory at t-1
    for t in T:
        model.addConstr(
            quicksum(l[depot, j, t] for j in N if (depot, j) in A) <= I[depot, t - 1],
    ...
    for t in T:
        for i in N:
            model.addConstr(u[i, t] <= n_cust * y[i, t], name=f"u_ub_{i}_{t}")
        for i in N:
            for j in N:
                if i == j:
                    continue
...
if __name__ == "__main__":
    ...
    model = build_irp_pd_mtz_from_excel(data)
    ...
    if model.status == GRB.OPTIMAL:
        print(f"\nOptimal solution (IRP-PD + MTZ, Excel input): {model.ObjVal}\n")
